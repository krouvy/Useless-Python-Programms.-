"""
This is same program which counts how
many times certain numbers
appear in the list, but another way
information output.
...
for i in range(len(Count_list)):  # then the loop goes through all index of the Num_list from 0 to 99
    if Count_list[i] > 0:  # and if element of Count_list > 0
        print((str(i) + " ") * Count_list[i], " ", end="")  # Convert the index to a string add "space"  and multiply
print("\nEnd")                                              # by the Count_list element, and print without line break
